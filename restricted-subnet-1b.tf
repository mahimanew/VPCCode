...
resource "aws_route_table" "StudentPlacement-Frankfurt-restricted-subnet-1b-router" {
  vpc_id = aws_vpc.StudentPlacement-Frankfurt-vpc.id
  ...
  route {
   ...
   tags = {
   Name = "StudentPlacement-Frankfurt-restricted-subnet-1b-router"
  }
}
...
resource "aws_route_table_association" "StudentPlacement-Frankfurt-restricted-1b-association" {
  ...
  route_table_id = aws_route_table.StudentPlacement-Frankfurt-restricted-subnet-1b-router.id
}